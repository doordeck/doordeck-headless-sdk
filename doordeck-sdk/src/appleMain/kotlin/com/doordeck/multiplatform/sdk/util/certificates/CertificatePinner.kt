...
/*
 * A fork of Ktor 3.5.1's io.ktor.client.engine.darwin.certificates.CertificatePinner, kept
 * byte-for-byte except for the Core Foundation ownership fixes marked "DOORDECK FIX" below.
 *
 * Upstream breaks the Core Foundation Get Rule: values obtained from CFDictionaryGetValue are owned
 * by the dictionary, and the two Security framework constants are owned by the framework, yet all of
 * them are passed to CFBridgingRelease. That is six over-releases per TLS handshake. The corrupted
 * objects are the values of the SecKeyCopyAttributes dictionary, so the process dies later - when the
 * Kotlin/Native GC finalizes that dictionary and -[__NSDictionaryM dealloc] releases values that are
 * already gone:
 *
 *     objc_object::release  <-  cow_cleanup  <-  -[__NSDictionaryM dealloc]
 *       <-  Kotlin_ObjCExport_releaseAssociatedObject  <-  GC finalizer processor
 *
 * Reproduced reliably on an Apple Watch Series 8 (arm64_32), where those CFString constants are not
 * represented the way they are on arm64. Not reproducible on the simulator, which never installs the
 * pinner. Verified fixed on-device with this file in place.
 *
 * Each fix retains the borrowed reference before handing ownership to CFBridgingRelease, leaving the
 * retain count balanced.
 */

package com.doordeck.multiplatform.sdk.util.certificates
...
public data class CertificatePinner(
    ...
    private fun SecCertificateRef.getPublicKeyBytes(): ByteArray? {
        ...
        return publicKeyRef.use {
            ...
            // DOORDECK FIX: Get Rule - the dictionary owns this value, so retain before bridging.
            ...
            // DOORDECK FIX: Get Rule - as above.
    ...
    private fun checkValidKeyType(publicKeyType: NSString, publicKeySize: NSNumber): Boolean {
        // DOORDECK FIX: these are Security framework constants; we do not own them.
...
/**
 * Copy of Ktor's internal `CPointer.use`, which this fork cannot reach from outside its module.
 */
@OptIn(ExperimentalForeignApi::class)
...
/**
 * Copies of Ktor's internal `NSData` bridges, unreachable from outside its module.
 */
